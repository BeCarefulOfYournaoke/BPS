...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if args.arch.startswith("vit"):
        lr = args.lr
        final_lr = args.lr * 0.001
        if args.cos:  # cosine lr schedule
            if epoch < args.warmup_epoch:
                lr = lr * (epoch + 1) / (args.warmup_epoch+1)
            else:
                lr = final_lr +  (lr - final_lr) * ( 0.5 * (1.0 + math.cos(math.pi * (epoch+1-args.warmup_epoch) / (args.epochs-args.warmup_epoch) ) ) )
        else: 
